...
if($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($c==1) {
        ...
        if($row["userType"]=="Student"){
            ...
            header("location: /~lipe4387/final/StudentPage.php");
        }else if($row["userType"]=="Faculty"){
            $_SESSION["Type"] = "Faculty";
            $_SESSION["ID"] = $row["userID"];
            header("location: /~lipe4387/final/FacultyPage.php");
        }else if($row["userType"]=="Admin"){
            $_SESSION["Type"] = "Admin";
            $_SESSION["ID"] = $row["userID"];
            header("location: /~lipe4387/final/AdminPage.php");
        }
   }else{
        echo "<script>alert('Wrong Password');location='/~lipe4387/final/login.php'</script>";
    }
    $mysqli->close();
}
?>
    ...
    <html>
        ...
        <body>
            <nav class="nav"><!-- Not sure if this page need this -->
                <h1>Navagation</h1>
                <ul>
                    <li><a href="/~lipe4387/final/login.php">Home</a></li><!-- Not sure what it could be -->
                </ul>
            </nav>
